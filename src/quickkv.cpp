...
void append_key_value(const FilePath &path, const Str &key, const Str &value) {
  ...
  if (!file.is_open()) {
    const auto msg =
        "Error in database::append_key_value(); can't open file: " +
        path.string();
    spdlog::error(msg);
...
SortedMap Database::last(const size_t count) const {
  ...
  auto last_view =
      data | std::views::drop(data.size() > count ? data.size() - count : 0);
...
Vec<Str> Database::keys(const FilterFunc &filter) const {
  ...
  std::ranges::copy_if(data | std::views::keys, std::back_inserter(key_list),
                       filter);
